...
function update_player()
	--player movement
		if btn(➡️) and not dead then
			if(ply.hs < ply.spd) ply.hs += ply.fric
		elseif btn(⬅️) and not dead then
			if(ply.hs > -ply.spd) ply.hs -= ply.fric
		else
	...
	--jumping
	if not dead and btnp(🅾️) and not ply.jumped then
	...
	if ply.jumped and not btn(🅾️) and ply.vs < 0 and ply.vs >= ply.jump then
		ply.vs *= ply.fric
	end
	...
	--dimension swapping
	if btnp(⬇️) and not ply.swapped then
		if eyes>1 then
			btnprompt=0
